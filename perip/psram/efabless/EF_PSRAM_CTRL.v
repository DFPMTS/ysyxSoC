...
module PSRAM_READER (
    ...
    wire [7:0]  FINAL_COUNT = 13 + size*2; // was 27: Always read 1 word // changed: -6 cycles
    ...
    wire[1:0] byte_index = {counter[7:1] - 8'd7}[1:0]; // changed: to match index
    always @ (posedge clk)
        if(counter >= 14 && counter <= FINAL_COUNT) // changed: read data from 14 cycle
            if(sck)
                data[byte_index] <= {data[byte_index][3:0], din}; // Optimize!

    // changed: all -6 cycles
    assign dout     =   (counter < 2)   ?   CMD_EBH[(1-counter)*4+:4]: // changed: cmd in QPI
    ...
    assign douten   = (counter < 8); // changed: -6 cycles
...
module PSRAM_WRITER (
    ...
    wire[7:0]        FINAL_COUNT = 7 + size*2; // changed: -6 cycles
    ...
    // changed: all -6 cycles
    assign dout     =   (counter < 2)   ?   CMD_38H[(1-counter)*4+:4]:
                        (counter == 2)  ?   saddr[23:20]        :
                        (counter == 3)  ?   saddr[19:16]        :
                        (counter == 4)  ?   saddr[15:12]        :
                        (counter == 5)  ?   saddr[11:8]         :
...
module PSRAM_ENTER_QPI (
  ...
  localparam IDLE = 1'b0, ENTER_QPI = 1'b1;
  reg state;
  reg nstate;
  reg [7:0] counter;
  ...
  always@(*)begin
    case(state)
      IDLE:      if(en) nstate = ENTER_QPI;
      ENTER_QPI: if(done) nstate = IDLE;
    endcase
  end
  ...
  assign dout = {3'b0, CMD_35H[3'd7-counter[2:0]]};
  assign douten = ~ce_n;
  assign done = counter == FINAL_COUNT + 1;
